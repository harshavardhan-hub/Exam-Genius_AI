...
/**
 * ExamGenius AI - Test Attempts Routes
 * Handles test attempt lifecycle and submissions
 */
...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: error.message || 'Failed to start test attempt' });
  } finally {
    client.release();
  }
});
...
router.post('/:attemptId/answer', auth, async (req, res) => {
  try {
    ...
    if (attemptResult.rows[0].completed) {
    ...
    await pool.query(`
      INSERT INTO user_answers (attempt_id, question_id, user_answer, is_correct, marks_obtained, time_taken_seconds)
      VALUES ($1, $2, $3, $4, $5, $6)
...
router.post('/:attemptId/finish', auth, async (req, res) => {
  ...
  try {
    ...
    `, [attemptId, attemptResult.rows[0].test_id]);
    ...
    const finalScore = Math.max(0, percentageScore); // Ensure non-negative score
    ...
    console.log(`✅ Test attempt completed: Score ${finalScore.toFixed(1)}`);
    ...
    res.status(500).json({ error: error.message || 'Failed to finish test attempt' });
  } finally {
    client.release();
  }
});
...
router.get('/:attemptId/report', auth, async (req, res) => {
  try {
    const { attemptId } = req.params;
    const userId = req.user.userId;
